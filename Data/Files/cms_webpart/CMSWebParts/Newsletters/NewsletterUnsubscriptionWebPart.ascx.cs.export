...
public partial class CMSWebParts_Newsletters_NewsletterUnsubscriptionWebPart : CMSAbstractWebPart
{
    ...
    protected void SetupControl()
    {
        ...
        {
            var subscriber = SubscriberInfoProvider.GetSubscriberInfo(subscriberGuid, siteId);
        ...
        if (unsubscribeFromAll)
        {
            ...
            {
               subscriptionService.UnsubscribeFromAllNewsletters(email, issueId, SendConfirmationEmail);
            }
        ...
        {
            var newsletter = NewsletterInfoProvider.GetNewsletterInfo(newsletterGuid, siteId);
            if (newsletter == null)
            {
                lblError.Visible = true;
    ...
    {
        var issue = IssueInfoProvider.GetIssueInfo(QueryHelper.GetInteger("issueid", 0)) ??
            IssueInfoProvider.GetIssueInfo(QueryHelper.GetGuid("issueguid", Guid.Empty), siteId);
